...
test("rendered human surface stays byte-stable after hidden metadata changes", async () => {
  ...
  const visibleSource = source.slice(sourceStart).replace(/<script[\s\S]*?<\/script>/gi, "").replace(/\s+/g, " ").trim();
  ...
  const visibleBody = html.slice(bodyStart, bodyEnd + "</body>".length).replace(/<script[\s\S]*?<\/script>/gi, "").replace(/\s+/g, " ").trim();
